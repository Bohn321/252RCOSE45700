...
@export var wave_time_add_per_wave := 2.0  # 每波额外增加多少秒
@export var wave_time_max := 60.0          # 可选：上限，防止无限长
...
func start_wave() -> void:
	...
	var extra := (wave_index - 1) * wave_time_add_per_wave
	wave_timer.wait_time = min(current_wave_data.wave_time + extra, wave_time_max)
